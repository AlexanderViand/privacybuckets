...
def compose(distribution_a, distribution_b, compositions=1):
    privacybuckets = ProbabilityBuckets(
        ...
        error_correction=False,  # error correction. See publication for details
    )
...
def main():
    ...
    # Define buckets
    binomial_buckets(int(n / 16), 1 + 1e-4, 300000)
    ...
    # plot(composed_bin.bucket_distribution, 'bucket distribution Binomial (n={})'.format(n), "Bucket Distributions",
    #      "bucket number", "mass")
    #
